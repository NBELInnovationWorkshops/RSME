...
class SimulationParameters(object):
    ...
    def __init__(self, simulation_parameters_xml, 
        ...
        """
        ## Morphology handling
        VSME assumes that the morphology is given as a 'hoc' file.
        ...
        self.logger.info('Layer 4 parsing concluded')
        ...
        if self.print_mode:
            print ('Simulation paremeters were loaded')
...
def layer1_parsing (element_tree):
    ...
    simdur      =     int(element_tree.find('{VSME}simulation_duration')   .text) # simulation time [ms]
    description =         element_tree.find('{VSME}simulation_description').text 
    
    return simdur, description

def layer2_parsing (element_tree, logger):
    
    stimuli_description_path = element_tree.find('{VSME}visual_stimulation').text.strip()
    tree = ET.parse("simulation_parameters/" + stimuli_description_path)
    root = tree.getroot().find('{VSME.simulation_parameters}stimulation')
    
    tgt_population  =     root.find('{VSME.simulation_parameters}tgt_population').text
    stimuli_field   = int(root.find('{VSME.simulation_parameters}field')         .text)
    frequancy       = int(root.find('{VSME.simulation_parameters}frequancy')     .text)
    blocked_field   = int(root.find('{VSME.simulation_parameters}blocked_field') .text)
...
def layer3_parsing (element_tree, logger):
    ...
    def parse_2d_grid(grid, layer_id):
        ...
        rect_grid = grid.find('{VSME.simulation_parameters}rectangular_location')

        rect_grid_corner = {'x': int(rect_grid.find('{VSME.simulation_parameters}corner').get('x')),
                            'y': int(rect_grid.find('{VSME.simulation_parameters}corner').get('y')),
                            'z': int(rect_grid.find('{VSME.simulation_parameters}corner').get('z'))}

        spacing = {'x': int(grid.find('{VSME.simulation_parameters}spacing').get('x')), 
                   'y': int(grid.find('{VSME.simulation_parameters}spacing').get('y'))}

        populations[population.get('cell_id')]['architecture'][layer_id] = {
            'spacing': spacing, 'rect_grid_corner': rect_grid_corner}

        n_cells = [int(rect_grid.find('{VSME.simulation_parameters}cells_number').get('x')),
                   int(rect_grid.find('{VSME.simulation_parameters}cells_number').get('y'))]
    ...
    morphology_description_path = element_tree.find('{VSME}network').text.strip()
    ...
    populations = {}
    population_element = root.find('{VSME.simulation_parameters}populations')
    ...
    for population in population_element:
        ...
        grid = population.find('{VSME.simulation_parameters}grid_arrangement')              
        if grid is not None:            
            populations[population.get('cell_id')]['type'] = 'grid_arrangement'
            parse_2d_grid(grid, 0) # 0 is the default index for mono-grid models
            define_neighbors()
            
        grid_layered = population.find('{VSME.simulation_parameters}layered_2d_grid')
        ...
        instances_element = population.find('{VSME.simulation_parameters}instances')
        ...
        if instances_element is not None:
            ...
            for instance in instances_element:
                populations[population.get('cell_id')]['cells'][cell_ind] = {}
                location = {
                    'x': instance.find('{VSME.simulation_parameters}location').get('x'),
                    'y': instance.find('{VSME.simulation_parameters}location').get('y'),
                    'z': instance.find('{VSME.simulation_parameters}location').get('z')}
    ...
    projections = {}
    projections_element = root.find('{VSME.simulation_parameters}projections')
    ...
    if projections_element is not None: 
        ...
        for projection in projections_element:
            ...
            connectivity_pattern_element = projection.find('{VSME.simulation_parameters}connectivity_pattern')

            x_y_intersection = connectivity_pattern_element.find('{VSME.simulation_parameters}x_y_intersection')

            if x_y_intersection is not None:

                x_y_alignment = x_y_intersection.find('{VSME.simulation_parameters}x_y_alignment')
            ...
            synapse = projection.find('{VSME.simulation_parameters}synapse')

            preffered_direction = synapse.find('{VSME.simulation_parameters}preffered_direction')
...
def layer4_parsing (element_tree, logger):
    ...
    morphology_description_path = element_tree.find('{VSME}morphology').text.strip()
    tree = ET.parse("simulation_parameters/" + morphology_description_path)
    root = tree.getroot()

    morphologies = {}
    morphology_element = root.find('{VSME.simulation_parameters}morphologies')
    for morphology in morphology_element:
        ...
        morphology_hoc_file = morphology.find('{VSME.simulation_parameters}morphology_hoc_file').text.strip()
        ...
        section_diameter = morphology.find('{VSME.simulation_parameters}section_diameter')
        diameter_fix = section_diameter.find('{VSME.simulation_parameters}math')
...
def layer5_parsing (element_tree, logger):
    ...
    biophysics_description_path = element_tree.find('{VSME}biophysics').text.strip()
    tree = ET.parse("simulation_parameters/" + biophysics_description_path)
    root = tree.getroot()
    
    biophysics_models = {}
    
    biophysics_element = root.find('{VSME.simulation_parameters}biophysics_models')
    cells_element = biophysics_element.find('{VSME.simulation_parameters}cells')
    for biophysics in cells_element:
        ...
        biophysics_models[biophysics.get('id')]['cytoplasmic_resistivity'] = (           
            int(biophysics.find('{VSME.simulation_parameters}cytoplasmic_resistivity').text))

        biophysics_models[biophysics.get('id')]['capicitance'] = (
            int(biophysics.find('{VSME.simulation_parameters}capicitance').text))
        
        # Parsing spatial distributios of channels
        spatial_distributions_element = biophysics.find('{VSME.simulation_parameters}channels_spatial_distribution')
        ...
        for channel_key in spatial_distributions_element:
            ...
            for channel in channel_key:
                ...
                model = channel.find('{VSME.simulation_parameters}math')
    ...
    synapse_element = biophysics_element.find('{VSME.simulation_parameters}synapses')
    syn_params_dict = {}
    for synapse in synapse_element:
        ...
        for synapse_property in synapse:
            ...
            model = synapse_property.find('{VSME.simulation_parameters}math')
